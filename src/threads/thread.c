...
static struct lock filesys_lock;  // for lock on files.
               ...
               thread_func *function, void *aux) 
{
  ...
  struct thread_child_data* c = malloc(sizeof(*c));
  c->tid = tid;
...
thread_exit (void) 
{
  ...
  /* Remove thread from all threads list, set our status to dying,
    ...
    while(!list_empty(&thread_current()->children)){
      struct proc_file *f = list_entry (list_pop_front(&thread_current()->children), struct thread_child_data, elem);
      free(f);
    }
